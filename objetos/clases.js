...
const myPenguin = {
	character: 'Opus',
	origin: 'Bloom County, Outland, Opus',
...
class Cuenta {
	...
	ingresar(cantidad){
		if(this.cantidad + cantidad > 900){
			return"Estas superando el limite de 900, Opecación cancelada"
		}else{
			this.cantidad += cantidad;
...
retirar (cantidad){
	if (this.cantidad-cantidad <10) {
		return "No tienes fondos suficientes, operación cancelada.";
	} else{
		this.cantifad -= cantidad;
		return`has retirado $       `
	}
}